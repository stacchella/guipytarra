...
c
      ...
      key = 'TARGCOOP'
      call ftpkyl(iunit,key,targcoopp,comment,status)
       if (status .gt. 0) then
          call printerror(status)
          print 10, key
       end if
...
c      
      ...
      key     = 'TARG_RA'
      call ftpkyd(iunit, key, targ_ra,12,comment,status)
      if (status .gt. 0) then
         call printerror(status)
         print *, 'TARG_RA'
      end if
...
c
      ...
      key     = 'TARG_DEC'
      call ftpkyd(iunit, key, targ_dec,12, comment,status)
      if (status .gt. 0) then
         call printerror(status)
         print *, 'TARG_DEC'
      end if
...
c     
      ...
      status =  0
c
c     comment='Number of A/D samples per pixel'
c      call ftpkyj(iunit,'NSAMPLES',nsamples,comment,status)
...
c
      ...
      call ftpkyd(iunit,'EFFEXPTM',exptime,-8,
     *     comment,status)
      if (status .gt. 0) then
         call printerror(status)
         print *, 'EXPTIME'
...
c     
      ...
      call ftpkyd(iunit,'RA_REF',ra_ref,-13,comment,status)
      if (status .gt. 0) then
         call printerror(status)
         print *, 'ra_ref'
      end if
...
c     
      ...
      call ftpkyd(iunit,'DEC_REF',dec_ref,-13,comment,status)
      if (status .gt. 0) then
         call printerror(status)
         print *, 'dec_ref'
      end if
