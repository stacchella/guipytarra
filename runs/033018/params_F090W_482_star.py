...
run_params_in = {'verbose': 1,             # verbose (f.e. 1)
                 ...
                 'filename': 'sim_cube_star_F090W_482_001.fits',  # output file name
                 ...
                 'sca_id': 482,               # ID of SCA to use  (f.e. 487)
                 'filter_index': 2,          # index of filter in 'nircam_calib.list' and 'psf.list'  (f.e. 10 for F200W)
                 'include_stars': True,      # include stars
                 'star_catalogue': 'star_cat_482.txt',    # catalog of stars, see below (f.e. none_487_090.cat)
                 'include_galaxies': False,    # include galaxies
                 'galaxy_catalogue': 'none.cat',  # catalog of galaxies, see below (f.e. mock_2017_11_03_487_090.cat)
